...
# 	print fromGlyph, toGlyph
	for layer in fromGlyph.layers:
		# Skip master layers
		if isLayerMasterLayer(layer): continue
		# Skip layers that don't belong to our desired master
		if fromFont.masters[layer.associatedMasterId].name != fromMaster['name']: continue
...
def copyMasterChildLayersToMaster(fromMasterIndex, toMasterIndex):
	...
	for i, fromGlyph  in enumerate(fromFont.glyphs):
		...
		if toGlyph: copyGlyphChildLayersToGlyph(fromGlyph, toGlyph)
...
class DialogWindow(object):
	def __init__(self):
		...
		self.w.myButton = Button((10, 110, -10, 20), "Copy Layers", callback=self.buttonCallback)
		self.w.open()
