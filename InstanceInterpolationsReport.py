...
for instance in Font.instances:
    ...
    for instanceInterpolation in instance.instanceInterpolations:
        text += "• %s: %s Master\n" % (
            "{0:.2f}".format(instance.instanceInterpolations[instanceInterpolation]),
            Font.masters[instanceInterpolation].name,
        )
    text += "\n"
...
class InstanceInterpolationsReport(object):
    ...
    def __init__(self):
        self.w = Window(
            (350, 600), "Instance Interpolations Report", minSize=(300, 450)
        )
        self.w.textEditor = TextEditor((10, 10, -10, -10), text=text, readOnly=True)
        self.w.open()
