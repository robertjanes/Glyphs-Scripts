...
def getListOfAllMastersGids():
	allMastersGids = []
	for master in allMasters: allMastersGids.append(master['gid'])
...
def copyAnchorsFromMasterToMaster(fromMasterIndex, toMasterIndex):
	...
	for i, fromGlyph  in enumerate(fromFont.glyphs):
		...
		if toGlyph: copyAnchorsFromLayerToLayer(fromGlyph.layers[fromMasterIndex], toGlyph.layers[toMasterIndex])
...
class DialogWindow(object):
	def __init__(self):
		...
		self.w.fromMasterSelect = PopUpButton((10, 30, -10, 20), getListOfAllMastersGids())
		self.w.toMasterLabel = TextBox((10, 60, -10, 17), "To Font Master")
		self.w.toMasterSelect = PopUpButton((10, 80, -10, 20), getListOfAllMastersGids())
		self.w.myButton = Button((10, 114, -10, 20), "Copy Anchors", callback=self.buttonCallback)
		self.w.open()

	def buttonCallback(self, sender):
		# Display error if the to/from masters are the same
		if self.w.fromMasterSelect.get() == self.w.toMasterSelect.get():
			Message("Copy anchors from master to master", "'From Master' and 'To Master' must be different!'")
			return
		self.w.close()
		# Run logic
		copyAnchorsFromMasterToMaster(self.w.fromMasterSelect.get(), self.w.toMasterSelect.get())
